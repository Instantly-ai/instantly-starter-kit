---
title: "Verification"
description: "Check deliverability before you send (credit-based)."
---

Check that an email address is deliverable **before** you send to it. Verifying protects your sender reputation and keeps bounce rates low — do it before adding leads to a campaign. Related: [leads](./leads) (verify on import), [campaigns](./campaigns) (`allow_risky_contacts`), [conventions → verify before you send](./conventions#verify-before-you-send).

> **Runs on Instantly Credits.** Verifying on import bulk-checks a whole list in one pass, so you spend credits deliberately instead of one address at a time. See [costs](./conventions) for what consumes credits.
...
For bulk, prefer verifying **on import** (`bulkAddLeads` with `verify_leads_on_import: true`) and reading list-level stats with `getVerificationStats` — see [leads](./leads).
...
## See also
Example: [`examples/03-add-leads-verify`](https://github.com/Instantly-ai/instantly-starter-kit/blob/main/js/examples). Template: [`outreach-service`](https://github.com/Instantly-ai/instantly-starter-kit/blob/main/js/templates/outreach-service) (verify-before-launch preflight), [`lead-pipeline`](https://github.com/Instantly-ai/instantly-starter-kit/blob/main/js/templates/lead-pipeline).
